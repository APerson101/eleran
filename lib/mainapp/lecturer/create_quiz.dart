          ...
          child: ElevatedButton(
              onPressed: () async {
                await showModalBottomSheet(
                  ...
                  context: context,
                  builder: (ctx) {
                    return MultiSelectBottomSheet<CourseModel>(
                      items: List<MultiSelectItem<CourseModel>>.generate(
                          courses.length,
                          (index) => MultiSelectItem(
                              courses[index], courses[index].name)),
                      initialValue: const [],
                      onConfirm: (values) {
...
final _selectedCourses =
    StateProvider.autoDispose<List<CourseModel>>((ref) => []);
